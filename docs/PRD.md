...
> **设计变更说明**：原设计采用离散步态（ADR-0001），经原型验证后改为连续移动（ADR-0003）。视角控制从独立模块改为 InputManager 统一管理（ADR-0005）。盲杖新增整根杖身碰撞体积（ADR-0004）。
...
挂载在 Player 下。管理盲杖的扫动、碰撞和 NPC 触发器。盲杖拥有整根杖身碰撞体积（ADR-0004）。
...
| C4 | 每帧做物理碰撞检测（RayCast3D 为主检测器） |
| C5 | 碰撞停杖：RayCast 检测到碰撞时盲杖视觉长度缩短到碰撞点，鼠标继续移但盲杖不动 |
| C6 | 碰撞时通过 EventBus 发送 `cane_hit_object` 信号 |
| C7 | 盲杖尖端挂 Area3D，用于 NPC 躲避检测 |
| C8 | Area3D 进入/离开 NPC 范围时通过 EventBus 发信号 |
| C9 | 整根杖身拥有 Area3D + BoxShape3D 碰撞体，防止杖身穿模；与墙壁/地形碰撞层检测，不影响玩家移动（"停杖不推人"） |
