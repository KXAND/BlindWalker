...
> 参考：`CONTEXT.md`（领域术语表）、`docs/adr/0001-discrete-stepping.md`  
> 最后更新：2026-07-04
...
BlindWalker 是一款模拟视障体验的第一人称公益游戏。玩家扮演视障者，通过**盲杖探测**、**离散步态行走**和**触摸确认**，从街角安全走到目的地。核心挑战来自离散迈步 + 台阶/墙壁物理判定的组合。
...
| 类别 | 常量 | 默认值 | 说明 |
|------|------|--------|------|
| 按键 | `KEY_LEFT_FOOT` | `KEY_W` | 左脚迈步 |
| 按键 | `KEY_RIGHT_FOOT` | `KEY_E` | 右脚迈步 |
| 按键 | `KEY_CAUTIOUS` | `KEY_SHIFT` | 谨慎模式 |
| 按键 | `KEY_HIGH_STEP` | `KEY_SPACE` | 抬腿模式 |
| 按键 | `KEY_LOOK_DIRECT` | `KEY_R` | 视角直控 |
| 按键 | `KEY_TOUCH` | `MOUSE_BUTTON_LEFT` | 触摸确认 |
| 步态 | `STEP_LENGTH_FLAT` | `0.5` | 平地步长 (m) |
| 步态 | `STEP_LENGTH_STAIR` | `0.35` | 楼梯步长 (m) |
| 步态 | `MAX_HIGH_STEP_HEIGHT` | `0.3` | 最大抬腿高度 (m) |
| 步态 | `HIGH_STEP_CHARGE_RATE` | `0.2` | 抬腿蓄力速度 (m/s) |
...
| `gait_state_changed` | `old_state: StringName, new_state: StringName` | 步态状态切换 |
...
### 2.2 GaitController（玩家 · 步态控制）

替代原 `PlayerController.gd`。挂载在 `CharacterBody3D` 上。

**需求**：

| ID | 需求 |
|----|------|
| G1 | W 键 = 左脚迈步。当前状态 BothEven 或 RightAhead 时，左脚迈出一步（固定位移） |
| G2 | E 键 = 右脚迈步。当前状态 BothEven 或 LeftAhead 时，右脚迈出一步（固定位移） |
| G3 | BothEven 状态下 W 或 E 皆可作为第一步起步 |
| G4 | 同脚连按：LeftAhead + W → 右脚收回到左脚侧（无位移，不摔跤），锁 W |
| G5 | 同脚连按：RightAhead + E → 左脚收回到右脚侧（无位移，不摔跤），锁 E |
| G6 | 锁脚后该键无效，必须用另一键解除锁定 |
| G7 | 按住 SHIFT + 迈步 = 谨慎模式，用于下楼梯 |
| G8 | 按住 SPACE + 迈步 = 抬腿模式，按住时长 = 抬腿高度 |
| G9 | 迈步时做物理判定：撞墙 → 踉跄（短暂趔趄反馈，不扣血） |
| G10 | 上楼梯：抬腿高度 < 台阶高度 → 摔跤，扣血 |
| G11 | 下楼梯：未按 SHIFT → 摔跤，扣血 |
| G12 | 平地步长与楼梯步长不同（可配置） |
| G13 | 步态状态（BothEven / LeftAhead / RightAhead / Locked）通过 EventBus 通知 |

---

### 2.3 CaneSystem（玩家 · 盲杖探测）

挂载在 Player 下。管理盲杖的扫动、碰撞和 NPC 触发器。
...
| C3 | 锥角内移动鼠标 → 只移盲杖，不转视角 |
| C4 | 每帧做物理碰撞检测（RayCast3D 或 PhysicsRayQuery） |
| C5 | 碰撞停杖：碰到障碍物时盲杖停在碰撞点，鼠标继续移但盲杖不动 |
| C6 | 碰撞时通过 EventBus 发送 `cane_hit_object` 信号 |
| C7 | 盲杖尖端挂 Area3D，用于 NPC 躲避检测 |
| C8 | Area3D 进入/离开 NPC 范围时通过 EventBus 发信号 |

---

### 2.4 ViewController（玩家 · 视角控制）

挂载在 Player 下。管理摄像机旋转。
...
| V2 | 盲杖锥角溢出时，继续推鼠标 → 旋转玩家 Yaw（视角 + 身体朝向同步） |
| V3 | 身体朝向 = 摄像机朝向，永远一致 |
...
AudioManager ← EventBus.audio_requested
CutsceneManager ← EventBus.cutscene_started/ended

GaitController ────→ EventBus (gait_state_changed, player_fell)
                  → PlayerAttributes (扣血)

CaneSystem ────→ EventBus (cane_hit_object, cane_entered/exited_npc_zone)

ViewController ─→ 从 CaneSystem 读取溢出状态 / 独立 R 键输入

TouchMemorySystem ─→ EventBus (touch_detected) [已有实现，暂不改动]

NPCBase ← NPCAvoidance (避让行为)
NPCAvoidance ← EventBus.cane_entered_npc_zone

InteractionSystem → EventBus (npc_interaction_available/unavailable)

StepResult / CaneHitInfo / NpcDialogue  ← GaitController, CaneSystem, InteractionSystem
```
