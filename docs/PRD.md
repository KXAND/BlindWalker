...
| 按键 | `KEY_CANE_POSTURE` | `KEY_R` | 扩展盲杖纵向姿态范围 |
...
| 杖触 | `CANE_TOUCH_MEMORY_COOLDOWN` | `0.75` | 同一点持续接触生成新记忆点的最小时间间隔 (s) |
| 血量 | `MAX_HP` | `100` | 最大血量 |
| 血量 | `FALL_DAMAGE` | `20` | 摔跤扣血 |
| 血量 | `STAIR_UP_DAMAGE` | `5` | 未按 SPACE 强行上台阶时扣血 |
...
| ID | 需求 |
|----|------|
| C1 | 鼠标 X/Y 轴控制盲杖左右/上下扫动；X 使用水平锥角，Y 默认使用点杖区间 A |
| C2 | 按住 R 时，Y 改为使用完整姿态区间 B；松开 R 后以当前实际姿态为中心建立新的 A，不发生跳变 |
| C3 | 当前范围内移动鼠标只移盲杖；超出部分返回 InputManager 旋转视角 |
| C4 | 每物理帧用 `intersect_shape`（BoxShape3D 全体积）预测新姿态是否与环境重叠，替代 RayCast 射线检测 |
| C5 | 碰撞停杖：新姿态重叠时旋转被阻止，杖身全长不缩短；分步推进确保快速扫动不跳过薄墙（ADR-0006） |
| C6 | 碰撞时通过 EventBus 发送 `cane_hit_object` 信号（`intersect_shape` 感知 + 射线/侧向采样定位接触点） |
...
| C15 | 收放动画以手端为枢轴，在 0.25s 内从上抬收纳姿态过渡到贴地工作姿态；过渡期间不得穿入脚下地面 |
| C16 | 盲杖 Pitch 与 Head Pitch 独立；盲杖到达 A/B 或碰撞边界后，溢出只旋转 Head，盲杖保持在实际边界 |
| C17 | Pitch 碰撞成为该方向的临时边界，目标姿态立即回到实际姿态，后续同向输入直接溢出到视角 |

---

### 2.4 ~~ViewController~~ → InputManager（玩家 · 输入与视角控制）

> 架构边界沿用 [ADR-0005](./adr/0005-input-manager-merges-view.md)：视角控制归入 InputManager，不再独立成模块；当前按键行为以下表为准。
...
| ID | 需求 |
|----|------|
| V1 | R 只切换盲杖纵向姿态区间 A/B，不绕过盲杖，也不改变水平扫杖规则 |
| V2 | 鼠标先驱动盲杖扫动，水平或纵向溢出量分别转为玩家 Yaw / Head Pitch 旋转 |
| V3 | 身体朝向 = 摄像机朝向，永远一致 |
...
CaneSystem ────→ EventBus (cane_hit_object, audio_requested, cane_entered/exited_npc_zone)
              → GameState.is_input_enabled()
              → TouchMemorySystem (spawn_touch_memory)

TouchMemorySystem ─→ Camera3D fullscreen post-process
